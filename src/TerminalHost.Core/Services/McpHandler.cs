...
public class McpHandler
{
    ...
    private IDebugLogService? _debugLog;
    ...
    // Maps Mcp-Session-Id → working directory (for memory repoId resolution)
    private readonly Dictionary<string, string> _sessionDirs = new();
    ...
    /// <summary>Wire in the debug log service (set after construction to avoid circular deps).</summary>
    public void SetDebugLog(IDebugLogService? debugLog) => _debugLog = debugLog;
    ...
    /// </summary>
    public async Task<McpResult> HandleRequestAsync(string jsonBody, string? sessionHint, string? mcpSessionId, string? workingDirHint = null, CancellationToken ct = default)
    {
        ...
        if (request.Method == "initialize")
        {
            ...
            var rootsDir = TryExtractRootsDir(request.Params) ?? workingDirHint;
            var newId = AssignSession(nameHint, rootsDir);
            sessionName = _sessionMap[newId];
            returnSessionId = newId;
            _debugLog?.Log("MCP", $"Initialize: session={sessionName}, id={newId}, roots={rootsDir}");
        }
        else if (!string.IsNullOrEmpty(mcpSessionId) && _sessionMap.ContainsKey(mcpSessionId))
        {
            ...
            // Late-populate working directory if not yet known (e.g., Channel bridge)
            if (!string.IsNullOrEmpty(workingDirHint))
            {
                lock (_sessionLock)
                {
                    _sessionDirs.TryAdd(mcpSessionId, workingDirHint);
                }
            }
        }
        ...
        {
            // No session ID or hint — try to find a named session with a working directory.
            // This handles Streamable HTTP clients that don't send Mcp-Session-Id on every request.
            var namedSession = _collab.GetSessions()
                .FirstOrDefault(s => !string.IsNullOrEmpty(s.WorkingDir) && !IsAutoGeneratedName(s.Name));
            sessionName = namedSession?.Name ?? "unknown";
            _debugLog?.Log("MCP", $"Session fallback: resolved to '{sessionName}' (mcpSessionId={mcpSessionId ?? "null"}, hint={sessionHint ?? "null"})");
        }

        // Register collab session with working directory (for discovery)
        var collabDir = request.Method == "initialize" ? (TryExtractRootsDir(request.Params) ?? workingDirHint) : null;
        ...
        if (request.Method == "tools/call")
            ...
            response = await HandleInitializeAsync(request, sessionName, ct);
        else
        {
    ...
    private string AssignSession(string? hint, string? rootsDir = null)
    {
        lock (_sessionLock)
        {
            ...
            _sessionMap[id] = name;
            if (!string.IsNullOrEmpty(rootsDir))
                _sessionDirs[id] = rootsDir;
            return id;
        }
    }

    /// <summary>
    /// Resolve the repository identifier for memory operations.
    /// Uses the working directory from the session, falling back to session name.
    /// </summary>
    private string ResolveRepoId(string session)
    {
        // Primary: check collab session's WorkingDir (survives across HTTP requests)
        var collabSession = _collab.GetSessions().FirstOrDefault(s => s.Name == session);
        if (collabSession != null && !string.IsNullOrEmpty(collabSession.WorkingDir))
            return collabSession.WorkingDir;

        lock (_sessionLock)
        {
            // Fallback: check name-keyed directory
            if (_sessionDirs.TryGetValue($"name:{session}", out var namedDir))
                return namedDir;

            // Fallback: find via MCP session ID mapping
            foreach (var kv in _sessionMap)
            {
                if (kv.Value == session && _sessionDirs.TryGetValue(kv.Key, out var dir))
                    return dir;
            }
        }
        return session;
    ...
    private async Task<JsonRpcResponse> HandleInitializeAsync(JsonRpcRequest request, string sessionName, CancellationToken ct)
    {
        var result = new McpInitializeResult();
        // Memory MCP tools are served by Eidet directly — TerminalHost does not proxy them.
    ...
    private JsonRpcResponse HandleToolsList(JsonRpcRequest request, string session)
    {
        ...
        var tools = GetToolDefinitions(needsIdentity);
        // Memory MCP tools are served by Eidet directly — not listed here.
    ...
    private async Task<McpCallToolResult> ExecuteToolAsync(string name, JsonElement args, string session, CancellationToken ct)
    {
        try
        {
            ...
            // Memory tools are served by Eidet directly — not routed here.
    ...
    private McpCallToolResult ExecuteSetSessionName(JsonElement args, string currentSession)
    {
        ...
        // Store working directory for memory repoId resolution
        if (!string.IsNullOrEmpty(workingDir))
        {
            lock (_sessionLock)
            {
                // Store under name key (reliable lookup even if MCP session ID changes)
                _sessionDirs[$"name:{name}"] = workingDir;

                // Also store under MCP session ID (for Channel bridge compatibility)
                foreach (var kv in _sessionMap)
                {
                    if (kv.Value == name)
                    {
                        _sessionDirs[kv.Key] = workingDir;
                        break;
                    }
                }
            }
