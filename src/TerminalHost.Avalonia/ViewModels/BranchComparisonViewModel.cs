...
/// <summary>
/// ViewModel for Branch Comparison popup.
/// Allows comparing two branches to see commits and files that differ.
/// </summary>
public partial class BranchComparisonViewModel : ObservableObject
{
    ...
    private async Task AssessMergeRiskAsync()
    {
        ...
        try
        {
            ...
            var result = await _aiExecutionService.ExecuteAsync(prompt, WorkingDirectory, "Assessing merge risk");

            if (result.Success && !string.IsNullOrWhiteSpace(result.Output))
                MergeRiskAssessment = result.Output.Trim();
        }
        finally
        {
    ...
    [RelayCommand]
    private async Task CopyMergeRiskAssessmentAsync()
    {
        if (!string.IsNullOrEmpty(MergeRiskAssessment))
        {
            await _clipboardService.SetTextAsync(MergeRiskAssessment);
