...
public partial class UnifiedGitPanelViewModel : BasePanelViewModel
{
    ...
    public override string PanelIcon => "Git";
    ...
    private async Task LoadTabDataAsync(GitPanelTab tab)
    {
        ...
        // Set IsOpen on the active sub-VM so its view renders,
        // and close inactive sub-VMs to hide their popup/overlay wrappers.
        BranchesVM.IsOpen = tab == GitPanelTab.Branches;
        ChangesVM.IsOpen = tab == GitPanelTab.Changes;
        HistoryVM.IsOpen = tab == GitPanelTab.History;
        StashVM.IsOpen = tab == GitPanelTab.Stash;
        TagsVM.IsOpen = tab == GitPanelTab.Tags;
        ComparisonVM.IsOpen = tab == GitPanelTab.Comparison;
