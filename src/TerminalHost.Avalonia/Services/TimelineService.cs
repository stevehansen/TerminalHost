...
/// <summary>
/// macOS/Avalonia implementation of the Timeline Mode service.
/// Ported from Core TimelineService with macOS-specific hook installation (curl instead of host.exe).
/// </summary>
public sealed class TimelineService : ITimelineService, IDisposable
{
    ...
    private Intent? FindIntentByWorkingDirectoryLocked(string workingDirectory)
    {
        ...
        {
            // Path might be a container-native Linux path on a macOS host
    ...
    public void HandleSessionStart(HookEvent hookEvent)
    {
        ...
        if (intent == null)
        {
            // Use Split for cross-platform safety (Linux paths on macOS host)
    ...
    #region Hook Installation (macOS — uses curl instead of host.exe)
