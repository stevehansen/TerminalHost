...
using SearchResult = TerminalHost.Domain.SearchResult;
...
internal sealed class SearchService : ISearchService
{
    ...
    async Task<SearchResults> ISearchService.SearchAsync(
        string pattern,
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Convert internal SearchResult to Core SearchResults format
        var searchResults = new SearchResults
        {
            SearchTimeMs = stopwatch.ElapsedMilliseconds,
            TotalMatchCount = results.Sum(r => r.Matches.Count)
        };
        ...
        foreach (var result in results)
        {
            ...
            {
                fileResult.Matches.Add(new Core.Domain.SearchMatch
                {
                    LineNumber = match.LineNumber,
                    Column = match.MatchStart,
                    MatchLength = match.MatchLength,
                    LineText = match.LineContent,
                    MatchedText = match.LineContent.Length > match.MatchStart + match.MatchLength
                        ? match.LineContent.Substring(match.MatchStart, match.MatchLength)
                        : ""
                });
            }

            searchResults.Files.Add(fileResult);
        }

        return searchResults;
    }

    async Task<int> ISearchService.ReplaceAsync(
        string pattern,
        string replacement,
        string workingDirectory,
        SearchOptions options,
        ...
        CancellationToken cancellationToken)
    {
        if (filesToReplace != null)
        {
            ...
            foreach (var file in filesToReplace)
            {
                ...
                totalReplacements += await ReplaceInFileAsync(
                    file,
                    pattern,
                    replacement,
                    options.CaseSensitive,
