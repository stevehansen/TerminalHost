...
internal sealed class GitWorktreeService : IGitWorktreeService
{
    ...
    #region IGitWorktreeService Implementation

    async Task<IReadOnlyList<WorktreeInfo>> IGitWorktreeService.ListWorktreesAsync(string repoPath)
    {
        return await GetWorktreesAsync(repoPath);
    }

    async Task<GitOperationResult> IGitWorktreeService.CreateWorktreeAsync(string repoPath, string branch, string targetPath, bool createBranch)
    {
        var result = await CreateWorktreeAsync(repoPath, targetPath, branch, createBranch);
        return new GitOperationResult { Success = result.Success, Error = result.Error };
    }

    async Task<GitOperationResult> IGitWorktreeService.RemoveWorktreeAsync(string worktreePath, bool force)
    {
        // Need to get the repository root first
        var repoRoot = await GetRepositoryRootAsync(worktreePath);
        if (string.IsNullOrEmpty(repoRoot))
            return new GitOperationResult { Success = false, Error = "Could not determine repository root" };

        var result = await RemoveWorktreeAsync(repoRoot, worktreePath, force);
        return new GitOperationResult { Success = result.Success, Error = result.Error };
    }

    async Task<GitOperationResult> IGitWorktreeService.PruneWorktreesAsync(string repoPath)
    {
        var result = await PruneWorktreesAsync(repoPath);
        return new GitOperationResult { Success = result.Success, Error = result.Error };
    }

    async Task<bool> IGitWorktreeService.IsWorktreeAsync(string path)
    {
        var worktrees = await GetWorktreesAsync(path);
        // If there's more than one worktree and this path is not the main one, it's a linked worktree
        var normalizedPath = Path.GetFullPath(path).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
        foreach (var wt in worktrees)
        {
            var normalizedWtPath = Path.GetFullPath(wt.Path).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
            if (normalizedWtPath.Equals(normalizedPath, StringComparison.OrdinalIgnoreCase))
            {
                return !wt.IsMain;
            }
        }
    ...
    async Task<string?> IGitWorktreeService.GetMainWorktreePathAsync(string worktreePath)
    {
        var worktrees = await GetWorktreesAsync(worktreePath);
        return worktrees.FirstOrDefault(w => w.IsMain)?.Path;
    }

    async Task<GitOperationResult> IGitWorktreeService.LockWorktreeAsync(string worktreePath, string? reason)
    {
        var result = await LockWorktreeAsync(worktreePath, reason);
        return new GitOperationResult { Success = result.Success, Error = result.Error };
    }

    async Task<GitOperationResult> IGitWorktreeService.UnlockWorktreeAsync(string worktreePath)
    {
        var result = await UnlockWorktreeAsync(worktreePath);
        return new GitOperationResult { Success = result.Success, Error = result.Error };
    }
