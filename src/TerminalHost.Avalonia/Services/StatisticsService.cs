    ...
    internal sealed class StatisticsService : IStatisticsService
    {
        ...
        public void RecordFocusTime(string directory, int seconds)
        {
            ...
            lock (_lock)
            {
                ...
                var today = DateTime.Now.ToString("yyyy-MM-dd");
                if (dirStats.FocusTimeByDay.ContainsKey(today))
                {
                    dirStats.FocusTimeByDay[today] += seconds;
                }
                else
                {
                    dirStats.FocusTimeByDay[today] = seconds;
        ...
        public long GetFocusTimeForPeriod(string directory, int days)
        {
            lock (_lock)
            {
                ...
                return dirStats.FocusTimeByDay
                    .Where(kvp => string.Compare(kvp.Key, cutoff, StringComparison.Ordinal) >= 0)
                    .Sum(kvp => kvp.Value);
            }
        }
        ...
        public long GetCharCountForPeriod(string directory, int days)
        {
            lock (_lock)
            {
                ...
                long total = 0;
                total += dirStats.CustomTerminalCharCountsByDay
                    .Where(kvp => string.Compare(kvp.Key, cutoff, StringComparison.Ordinal) >= 0)
                    .Sum(kvp => kvp.Value);
                total += dirStats.ShellTerminalCharCountsByDay
                    .Where(kvp => string.Compare(kvp.Key, cutoff, StringComparison.Ordinal) >= 0)
                    .Sum(kvp => kvp.Value);
                total += dirStats.RunTerminalCharCountsByDay
                    .Where(kvp => string.Compare(kvp.Key, cutoff, StringComparison.Ordinal) >= 0)
                    .Sum(kvp => kvp.Value);
