...
def main():
    ...
    if pid == 0:
        ...
        # Ensure we're in a valid working directory (not inside app bundle or DMG)
        # Only change to home if current directory is truly invalid
        try:
            cwd = os.getcwd()
            # Only these patterns indicate we're in an invalid location
            invalid_patterns = ['/Volumes/', '.app/Contents/']
            is_invalid = any(pattern in cwd for pattern in invalid_patterns)
