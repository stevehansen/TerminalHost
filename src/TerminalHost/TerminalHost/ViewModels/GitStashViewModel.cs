...
public partial class GitStashViewModel : ObservableObject
{
    ...
    private async Task CreateStashAsync()
    {
        ...
        try
        {
            var (success, error) = await _gitStatusService.StashAsync(
    ...
    private async Task ApplyStashAsync()
    {
        ...
        try
        {
            var (success, error) = await _gitStatusService.StashApplyAsync(
                CurrentWorkingDirectory,
                SelectedStash.Index);
    ...
    private async Task PopStashAsync()
    {
        ...
        try
        {
            var (success, error) = await _gitStatusService.StashPopAsync(
                CurrentWorkingDirectory,
                SelectedStash.Index);
    ...
    private async Task DropStashAsync()
    {
        ...
        if (!_dialogService.ShowConfirmation(
            $"Are you sure you want to drop '{SelectedStash.DisplayName}'?\n\nThis action cannot be undone.",
            "Drop Stash"))
            return;
        ...
        try
        {
            var (success, error) = await _gitStatusService.StashDropAsync(
                CurrentWorkingDirectory,
                SelectedStash.Index);
    ...
    private async Task CreateBranchFromStashAsync()
    {
        ...
        try
        {
            var (success, error) = await _gitStatusService.StashBranchAsync(
                CurrentWorkingDirectory,
                SelectedStash.Index,
                NewBranchName.Trim());
