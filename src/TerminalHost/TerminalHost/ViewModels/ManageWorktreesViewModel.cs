...
public partial class ManageWorktreesViewModel : ObservableObject
{
    ...
    public async Task OpenAsync()
    {
        ...
        var repoRoot = await _gitWorktreeService.GetRepositoryRootAsync(workingDir);
    ...
    private async Task RefreshAsync()
    {
        ...
        try
        {
            var worktreeInfos = await _gitWorktreeService.GetWorktreesAsync(RepoPath);
            Worktrees.Clear();
    ...
    private async Task RemoveWorktreeAsync(WorktreeInfo? worktree)
    {
        ...
        try
        {
            var result = await _gitWorktreeService.RemoveWorktreeAsync(RepoPath, worktree.Path, force: false);
            if (result.Success)
            {
                await RefreshAsync();
            }
            else
            {
                ...
                if (forceConfirmed)
                {
                    result = await _gitWorktreeService.RemoveWorktreeAsync(RepoPath, worktree.Path, force: true);
                    if (result.Success)
                    {
                        await RefreshAsync();
                    }
