...
public partial class TimelineTabViewModel : ObservableObject, ITabViewModel
{
    ...
    private void OnCurrentIntentChanged(object? sender, string? intentId)
    {
        ...
        {
            SelectedIntent = Intents.FirstOrDefault(i => i.Id == intentId);
        }
    }
    ...
    [RelayCommand]
    private async Task BrowseForRepoPath()
    {
        var path = await _folderPickerService.PickFolderAsync("Select Repository");
