...
public class LiveSessionTrackerTests
{
    ...
    [Fact]
    public void ActivityEventProcessed_BumpsLastActivityTime_OnLiveSession()
    {
        // SessionActivityService is the canonical source of activity timestamps.
        // LiveSessionTracker mirrors them onto LiveSession via the subscription.
        var activityService = new SessionActivityService();
        _store.Setup(s => s.EnsureIntentForWorkingDirectory(It.IsAny<string>(), It.IsAny<string>()))
            .Returns<string, string>((cwd, name) => Intent.Create(name, "main", cwd));
        ...
        var before = tracker.GetLiveSessionByClaudeId("sess-1")!.LastActivityTime;

        // Drive activity through the activity service — ToolEnd fires ActivityEventProcessed
        // which the tracker subscribes to.
        activityService.ProcessHookEvent(new HookEvent
        {
            SessionId = "sess-1",
        ...
        var live = tracker.GetLiveSessionByClaudeId("sess-1")!;
        live.LastActivityTime.ShouldNotBeNull();
        if (before.HasValue)
            live.LastActivityTime!.Value.ShouldBeGreaterThanOrEqualTo(before.Value);
    }

    [Fact]
    public void ActivityEventProcessed_IgnoresSessionLifecycleEvents()
    {
        // SessionStart/SessionEnd events should NOT bump LastActivityTime —
        // we want the longer "no-activity-ever" timeout window to apply until
        ...
        tracker.HandleSessionStart(SessionStart("sess-1", Path.GetTempPath()));
        // HandleSessionStart triggers activity service via consumer's call path normally,
        // but the tracker does not call it directly. So LastActivityTime should be null.
        tracker.GetLiveSessionByClaudeId("sess-1")!.LastActivityTime.ShouldBeNull();

        // Even if SessionStart fires through activity service, it should be ignored.
        activityService.ProcessHookEvent(new HookEvent
        {
            SessionId = "sess-1",
            EventType = HookEventType.SessionStart,
